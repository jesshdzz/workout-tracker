// app/features/dashboard/components/BlockProgress.tsx
...
export function BlockProgress({ currentWeek }: Props) {
  ...
  return (
    <div className="p-4 space-y-3 rounded-2xl bg-surface">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-xs tracking-widest uppercase text-muted">Bloque {config.block}</p>
        ...
        <div className="text-right">
          ...
          <p className="font-mono text-3xl font-bold text-white">{currentWeek}</p>
        </div>
      </div>

      <Progress value={progress} className="h-1.5 bg-bg" />
      ...
      {config.isDeload && (
        <div className="px-3 py-2 border rounded-lg bg-danger/10 border-danger/20">
          <p className="text-xs font-medium text-danger">
            ⚠️ Semana de deload — Volumen reducido 40%
          </p>
        </div>
      )}
    </div>
